"""
Example: Modflow 6 model of the Hondsrug
*****************************************

This example is a case study area in the Netherlands, 
in the province of Drenthe, near an ice-pushed ridge, called "Hondsrug". 
This is a complexer model, which includes the use 
of external data as input data including the necessary 
modifications so it can be used in the model. 

For more information about MODFLOW 6: 
...
# TODO: Check if warning filter still needed
import warnings
...
# =======================
#
# Before starting to create the input data,
# we will create the groundwater model variable (gwf_model) using
# `imod.mf6.GroundwaterFlowModel <https://imod.xyz/api/mf6.html?highlight=groundwater%20flow%20model#imod.mf6.GroundwaterFlowModel>`_.
# The data from all the model packages will be added to this variable.

gwf_model = imod.mf6.GroundwaterFlowModel()

# %%
# Structured discretization package - DIS
# ========================================
#
# This package allows specifying a regular MODFLOW grid.
# This grid is assumed to be rectangular horizontally,
# but can be distorted vertically.
#
# Load data
# ---------
#
...
idomain = xr.open_dataarray(wdir / "idomain.nc")
top = xr.open_dataarray(wdir / "top.nc")
bot = xr.open_dataarray(wdir / "bot.nc")

# %%
# Adding information to the DIS package
# -------------------------------------
#
# The following step is to add the previously created discretization data to the gwf_model variable.
# This is done using the function
# `imod.mf6.StructuredDiscretization <https://imod.xyz/api/mf6.html?highlight=structured%20discretization#imod.mf6.StructuredDiscretization>`_.
# The data to include is the top of the model domain, the bottom of the layers, and the idomain.
# All this information comes from the previously imported tifs (now converted to
# `xarray.DataArray <http://xarray.pydata.org/en/stable/generated/xarray.DataArray.html#xarray.DataArray>`_.
...
# TODO: Add data to pooch, and load in that manner.
k = xr.open_dataarray(wdir / "k.nc")

# %%
# ICELLTYPE
# ----------
#
# The cell type to be used in the model (confined or convertible)
# can be defined under ICELLTYPE, which is an input to the NPF package.
...
#
# In this example, all layers have a ICELLTYPE equal to 0, indicating confined cells.
...
# `imod.mf6.NodePropertyFlow <https://imod.xyz/api/mf6.html?highlight=structured%20discretization#imod.mf6.NodePropertyFlow>`_.
# The information included is the icelltype value (equal to zero),
# the array for  the hydraulic conductivity
# (considered to be the same for the horizontal and vertical direction) and, optionally,
# the variable_vertical_conductance, dewatered, perched and save_flows options have been activated.
# For more details about the meaning of these variables and other variables
# available to be used within this package, please refer to the
# `documentation <https://imod.xyz/api/mf6.html?highlight=structured%20discretization#imod.mf6.NodePropertyFlow>`_.
...
# ----------------------------
#
# The starting heads to be used in this model are based on the
# interpolation of x-y head
# measurements, which were interpolated on a larger area.
# This example was created in this example
# --insert-link-here--
#
# The heads were interpolated on a larger area,
# therefore these have to be clipped first

# TODO: Add data to pooch, and load in that manner.
interpolated_head_larger = xr.open_dataarray(wdir / "interpolated_head_larger.nc")
...
# TODO: Add data to pooch, and load in that manner.
pp = xr.open_dataarray(wdir / "RDH_REF.nc")
evt = xr.open_dataarray(wdir / "EV24.nc")

pp = pp.sel(x=slice(xmin, xmax), y=slice(ymax, ymin)) / 1000  # to meters
evt = evt.sel(x=slice(xmin, xmax), y=slice(ymax, ymin)) / 1000  # to meters
...
# TODO: Add data to pooch, and load in that manner.
tube_cond = xr.open_dataarray(wdir / "tube_cond.nc")
tube_elev = xr.open_dataarray(wdir / "tube_elev.nc")
...
gwf_model["drn-tube"] = imod.mf6.Drainage(conductance=tube_cond, elevation=tube_elev)
...
# TODO: Add data to pooch, and load in that manner.
riv_cond = xr.open_dataarray(wdir / "riv_cond.nc")
riv_stage = xr.open_dataarray(wdir / "riv_stage.nc")
riv_bot = xr.open_dataarray(wdir / "riv_bot.nc")
