...
layermin = idomain.coords["layer"].min().values[()]
layermax = idomain.coords["layer"].max().values[()]
...
simulation.write(modeldir, False, False)

regridded_simulation = simulation.regrid_like(
    "hondsrug-regridded", target_grid, validate=False
)
regridded_simulation.write(modeldir, False, False)
