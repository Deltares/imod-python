...
def create_transport_model (flowmodel, speciesname, dispersivity, retardation, decay):
    ...
    if retardation != 1.0:
        sorption = "linear"
        kd = (
            (retardation - 1.0) * porosity / rhobulk
        )
    else:
        sorption = None
        kd = 1.0
...
constant_conc = constant_conc.expand_dims(species=["species_a", "species_b", "species_c", "species_d"])
...
gwf_model["discretization"] = imod.mf6.StructuredDiscretization(
    top=0.0, bottom=bottom, idomain=grid,
)
