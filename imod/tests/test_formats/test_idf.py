...
def _save_subdomains_no_species(subdomains, tmp_path, start_nr=0):
    for i, subdomain in enumerate(subdomains):
        ...
        for layer, da in subdomain.groupby("layer"):
            idf.write(tmp_path / f"subdomains_20000101_l{layer}_p00{nr}.idf", da.squeeze(drop=True))
    ...
    for i, subdomain in enumerate(subdomains):
        for species, das in subdomain.groupby("species"):
            for layer, da in das.groupby("layer"):
                idf.write(
                    tmp_path / f"subdomains_c{species}_20000101_l{layer}_p00{i}.idf", da.squeeze(drop=True)
                )
