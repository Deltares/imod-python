...
import xugrid as xu
import numpy as np
from imod.tests.fixtures.mf6_regridding_fixture import grid_data_structured, grid_data_structured_layered
...
#make new grid
    ...
    new_gwf_model = gwf_model.regrid_like(finer_idomain)
    assert len(new_gwf_model.items()) == len (gwf_model.items())
