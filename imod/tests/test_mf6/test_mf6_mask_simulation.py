...
def test_mask_simulation_different_domains(
    structured_flow_simulation_2_flow_models: imod.mf6.Modflow6Simulation,  tmp_path: Path,
):
    ...
    new_grid = xr.DataArray(
        np.ones(shape, dtype=float) * 1, coords=coords, dims=dims
    )
    ...
    with pytest.raises(ValueError , match="when all the models in the simulation use the same grid."):
