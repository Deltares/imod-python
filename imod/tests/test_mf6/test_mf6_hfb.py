...
def test_to_mf6_layered_hfb__error_non_unique_layer(geometry_line_triple):
    """Throws error because multiple layers attached to one object."""
    # Arrange.
    geometry_line_triple["layer"] = [1, 1, 2]  # make layers non-unique

    hfb = SingleLayerHorizontalFlowBarrierResistance(geometry_line_triple)
    errors = hfb._validate(hfb._write_schemata)

    assert len(errors) > 0


def test_to_mf6_layered_hfb__error_geometry_type(geometry_polygon):
    """Throws error because wrong geometry type in object."""
    # Arrange.
    print_input = False

    hfb = SingleLayerHorizontalFlowBarrierResistance(geometry_polygon, print_input)
    errors = hfb._validate(hfb._write_schemata)
...
def test_is_empty(geometry_polygon):
    ...
    hfb = HorizontalFlowBarrierResistance(empty_geometry)
    assert hfb.is_empty()

    hfb = HorizontalFlowBarrierResistance(geometry_polygon)
    assert not hfb.is_empty()


@pytest.mark.parametrize(
    "parameterizable_basic_dis",
    [BasicDisSettings(nlay=2, nrow=3, ncol=3, xstart=0, xstop=12, ystart=0, ystop=8)],
    indirect=True,
)
@pytest.mark.parametrize("print_input", [True, False])
def test_set_options(print_input, parameterizable_basic_dis, geometry_polygon):
...
def test_hfb_from_imod5__dbase_fixture(imod5_dataset, tmp_path):
...
def test_hfb_from_imod5__simple_3D(geometry_polygon):
    imod5_data = {"hfb-1": {"geodataframe": geometry_polygon, "layer": 0}}
    hfb = HorizontalFlowBarrierResistance.from_imod5_data("hfb-1", imod5_data)
    assert hfb.dataset["geometry"].size == len(geometry_polygon)
    imod5_data = {"hfb-1": {"geodataframe": geometry_polygon, "layer": 1}}
    with pytest.raises(
        ValueError, match="Try SingleLayerHorizontalFlowBarrierResistance class"
    ):
        _ = HorizontalFlowBarrierResistance.from_imod5_data("hfb-1", imod5_data)


def test_hfb_from_imod5__simple_2D(geometry_line_single):
    imod5_data = {"hfb-1": {"geodataframe": geometry_line_single, "layer": 1}}
    hfb = SingleLayerHorizontalFlowBarrierResistance.from_imod5_data(
        "hfb-1", imod5_data
    )
    assert hfb.dataset["geometry"].size == len(geometry_line_single)
    imod5_data = {"hfb-1": {"geodataframe": geometry_line_single, "layer": 0}}
    with pytest.raises(ValueError, match="Try HorizontalFlowBarrierResistance class"):
        _ = SingleLayerHorizontalFlowBarrierResistance.from_imod5_data(
            "hfb-1", imod5_data
        )


def test_snap_to_grid_and_aggregate(structured_flow_model, geometry_line_triple):
    ...
    snapped_dataset, edge_index = _snap_to_grid_and_aggregate(
        geometry_line_triple, grid2d, vardict_agg
    )
...
def test_snap_to_grid_public(structured_flow_model, geometry_line_triple):
    dis = structured_flow_model["dis"]

    geometry_triple = geometry_line_triple
    hfb_triple = SingleLayerHorizontalFlowBarrierResistance(geometry_triple)
    snapped_dataset = hfb_triple.snap_to_grid(dis)
...
def test_combine_linestrings(
    structured_flow_model, geometry_line_single, geometry_line_triple
):
    dis = structured_flow_model["dis"]
    top, bottom, idomain = (
        dis["top"],
    ...
    hfb_single = SingleLayerHorizontalFlowBarrierResistance(geometry_line_single)
    hfb_triple = SingleLayerHorizontalFlowBarrierResistance(geometry_line_triple)
...
def test_run_multiple_hfbs(tmp_path, structured_flow_model, geometry_line_single):
    ...
    structured_flow_model["hfb"] = SingleLayerHorizontalFlowBarrierResistance(
        geometry_line_single
    )
    ...
    geometry_line_single["resistance"] = 400.0
...
def test_make_linestring_from_polygon(geometry_polygon):
    linestrings = _make_linestring_from_polygon(geometry_polygon)

    coordinates = get_coordinates(linestrings)

    barrier_x = [5.0, 5.0, 5.0, 5.0]
    barrier_y = [11.0, 5.0, 5.0, -1.0]
...
def test_extract_hfb_bounds_from_dataframe__fails(geometry_line_triple):
    """Test if function throws error when providing a line."""

    with pytest.raises(TypeError):
        _extract_mean_hfb_bounds_from_dataframe(geometry_line_triple)


def test_custom_deepcopy(geometry_line_triple):
    # Arrange
    hfb = SingleLayerHorizontalFlowBarrierResistance(geometry_line_triple)
...
def test_clip_box(geometry_line_single):
    # Arrange
    hfb = SingleLayerHorizontalFlowBarrierResistance(geometry_line_single)
