...
from imod.mf6.modelsplitter import (
    split_model_packages
)
import imod
import numpy as np
...
def test_partition_structured(twri_model):
    ...
    partitioning_array = copy.deepcopy(twri_model["GWF_1"]["dis"]["idomain"].sel({"layer": 1}))
    ...
    unique_labels, label_counts = np.unique(partitioning_array.values, return_counts=True)
    for ilabel in unique_labels:
        imodel_idomain = new_models[ilabel - 1]["dis"]["idomain"].sel({"layer": 1}).values
        active_unique, active_count = np.unique(imodel_idomain,  return_counts=True)
        assert label_counts[ilabel-1] == active_count[0]

@pytest.mark.usefixtures("circle_model")
def test_partition_unstructured(circle_model, tmp_path):
    partitioning_array = copy.deepcopy(circle_model["GWF_1"]["disv"]["idomain"].sel({"layer": 1}))
