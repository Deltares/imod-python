...
def create_gridcovering_array(idomain, lake_cells, fillvalue, dtype):
    result = xr.full_like(idomain, fill_value=lake_api.missing_values[np.dtype(dtype).name], dtype=dtype)
...
def create_lakelake(idomain, starting_stage, boundname, lake_cells):
    connectionType = create_gridcovering_array(idomain, lake_cells, lake_api.connection_types["HORIZONTAL"], np.int32)
    ...
    result = lake_api.LakeLake(starting_stage,boundname,connectionType, bed_leak, top_elevation, bot_elevation,
        connection_length, connection_width, None,None, None, None, None, None, None, None, None)
    return result
...
rate1 = xr.DataArray(np.ones(nconnect, dtype=np.float32), coords=coordinates, dims=dimensions)
rate2 = xr.DataArray(np.zeros(nconnect, dtype=np.float32), coords=coordinates, dims=dimensions)
...
lake2 = create_lakelake(idomain, 15, "Ijsselmeer", [(1,12,12), (1,12,13),(1,13,13)])
