...
class ImmobileStorage(Package):
    ...
    budgetcsvfile:
        name of the comma-separated value (CSV) output file to write budget summary information. A budget
        summary record will be written to this file for each time step of the simulation.
    sorption: ({True, False}, optional)
        is a text keyword to indicate that sorption will be activated. Use of this keyword requires that
        BULK_DENSITY and DISTCOEF are specified in the GRIDDATA block. The linear sorption isotherm is the only
         isotherm presently supported in the IST Package.
    first_order_decay: ({True, False}, optional)
        is a text keyword to indicate that first-order decay will occur. Use of this keyword requires that DECAY and
        DECAY_SORBED (if sorption is active) are specified in the GRIDDATA block.
    zero_order_decay: ({True, False}, optional)
        is a text keyword to indicate that zero-order decay will occur. Use of this keyword requires that DECAY and
         DECAY_SORBED (if sorption is active) are specified in the GRIDDATA block.
    cimfile: (str)
        name of the output file to write immobile concentrations. This file is a binary file that has the same format
        and structure as a binary head and concentration file. The value for the text variable written to the file is
        CIM. Immobile domain concentrations will be written to this file at the same interval as mobile domain
    ...
    cim : array of floats (xr.DataArray)
        initial concentration of the immobile domain in mass per length cubed. If CIM is not specified,
        then it is assumed to be zero.
    thetaim : array of floats (xr.DataArray)
        porosity of the immobile domain specified as the volume of immobile pore space per total volume (dimensionless).
    zetaim: array of floats (xr.DataArray)
    ...
    decay: array of floats (xr.DataArray)
        is the rate coefficient for first or zero-order decay for the aqueous phase of the immobile domain.
        A negative value indicates solute production. The dimensions of decay for first-order decay is one over time.
        The dimensions of decay for zero-order decay is mass per length cubed per time.
        Decay will have no effect on simulation results unless either first- or zero-order decay is
    ...
    bulk_density: array of floats (xr.DataArray)
        is the bulk density of the aquifer in mass per length cubed. bulk_density will have no effect on
        simulation results unless the SORPTION keyword is specified in the options block.
    distribution_coefficient: array of floats (xr.DataArray)
        is the distribution coefficient for the equilibrium-controlled linear sorption isotherm
    ...
    def __init__(
        ...
        if sorption is True:
            if bulk_density is None or distribution_coefficient is None:
                warnings.warn(
                    "if sorption is active, a bulk density and distribution coefficient must be provided.",
                    FutureWarning,
                )
        if first_order_decay is True or zero_order_decay is True:
            if decay is None:
                warnings.warn(
                    "if first_order_decay is active or if zero_order_decay is active, decay must be provided.",
                    FutureWarning,
                )
            if sorption is True:
