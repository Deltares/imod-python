...
class LakeLake:
    ...
    def get_1d_array(cls, grid_array):
        ...
        dummy = dummy.assign_coords({"x": x_indexes, "y": y_indexes, "layer": layer_indexes})
...
def from_lakes_and_outlets( list_of_lakes, list_of_outlets = []):
    ...
    for i in range(0, nrlakes):
        ...
        layer, y, x, ctype = list_of_lakes[i].get_1d_array(list_of_lakes[i].connectionType)
        c_type += ctype
        ...
        c_bed_leak +=bed_leak
        _, _, _, top_elevation = list_of_lakes[i].get_1d_array(list_of_lakes[i].top_elevation)
        c_top_elevation +=top_elevation
        _, _, _, bot_elevation = list_of_lakes[i].get_1d_array(list_of_lakes[i].bot_elevation)
        c_bottom_elevation+=bot_elevation
        _, _, _, connection_length = list_of_lakes[i].get_1d_array(list_of_lakes[i].connection_length)
        c_length+=connection_length
        _, _, _, connection_width = list_of_lakes[i].get_1d_array(list_of_lakes[i].connection_width)
