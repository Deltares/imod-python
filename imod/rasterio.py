...
def resample(
    ...
    """
    Function to reprojects and/or resamples a 2D xarray DataArray to a 
    ...
    Reproject a DataArray from one coordinate system (WGS84, EPSG:4326) to another (UMT30N, EPSG:EPSG:32630):
    ...
    Open a raster, and reproject to RD new coordinate system (EPSG:28992), without explicitly specifying `src_crs`.
    `src_crs` is taken from `a.attrs`, so the raster file has to include coordinate system information for this to work.
    >> a = xr.open_rasterio("example.tif")
    ...
    >> rotated = xr.open_rasterio("rotated_example.tif")
