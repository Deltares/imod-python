...
def save(path, a, nodata=1.0e20, pattern=None):
    ...
    pattern : str
        Format string which defines how to create the filenames. Check the
        examples.
    ...
    default filenames would be generated by the followin format string:

        save("example", pattern="{name}_l{layer}{extension}")

    This generates layers with as many digits as for layer. If you desire padded
    numbers so they show up neatly sorted in a file manager, you may specify:

        save("example", pattern="{name}_l{layer:02d}{extension}")
    
    In this case, a 0 will be padded for single digit numbers (l1 will become
    l01).
