...
Copied from: https://github.com/Deltares/xugrid/blob/connectivity_utils/xugrid/snapping.py
Can be removed later when released in the xugrid package.
"""
...
def snap_to_grid(
    ...
    # line_index now maps line points to line index,
    # we need to map line segments to line index
    # For two lines consisting both of two segments:
    # [0, 0, 0, 1, 1, 1] needs to be turned into [0, 0, 1, 1]
    select = np.append(np.diff(line_index), 1) == 0
    line_index = line_index[select]

    snapped_line_index = line_index[segment_index]
    ...
    if return_geometry:
        ...
        gdf = gpd.GeoDataFrame(
            lines.drop(columns="geometry").iloc[snapped_line_index], geometry=geometry
        )
        return cell_to_cell, gdf
    else:
        df = lines.drop(columns="geometry").iloc[snapped_line_index]
