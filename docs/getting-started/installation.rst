...
When using a conda Python, it is recommended to use (fast)::

    mamba install -c conda-forge imod
  
or (slower)::

    conda install -c conda-forge imod
 
The ``imod`` Python package can also be installed from the `Python Package
Index`_ with::
  
    pip install imod
    
If these instructions mean nothing to you, read the sections below.
